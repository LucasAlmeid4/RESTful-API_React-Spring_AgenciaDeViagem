import './FormContatoCss.css'


function FormContato() {
    return(
        <section className="text-center">

        <h1 className='my-5'>Entre em contato conosco</h1>
        <hr/>
        <div className='form-bg'>
        <h3 className='my-5'>Preencha os campos abaixo ou ligue:</h3>
        <h4><svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" class="bi bi-telephone-fill" viewBox="0 0 16 16">
                                                                <path fill-rule="evenodd" d="M1.885.511a1.745 1.745 0 0 1 2.61.163L6.29 2.98c.329.423.445.974.315 1.494l-.547 2.19a.678.678 0 0 0 .178.643l2.457 2.457a.678.678 0 0 0 .644.178l2.189-.547a1.745 1.745 0 0 1 1.494.315l2.306 1.794c.829.645.905 1.87.163 2.611l-1.034 1.034c-.74.74-1.846 1.065-2.877.702a18.634 18.634 0 0 1-7.01-4.42 18.634 18.634 0 0 1-4.42-7.009c-.362-1.03-.037-2.137.703-2.877L1.885.511z"/>
                                                                </svg>+55 11 77070-8080</h4>
        
        <form className="alinha">

            <div className="form-outline mb-4">
                <label className="form-label" for="form4Example1">Nome:</label>
                <input type="text" id="form-nome" className="form-control" />

            </div>

    
            <div className="form-outline mb-4">
                <label className="form-label" for="form4Example2">Email:</label>
                <input type="email" id="form-email" className="form-control" />

            </div>

            
            <div className="form-outline mb-4">
                <label className="form-label" for="form4Example3">Assunto:</label>
                <textarea className="form-control" id="assunto" rows="4"></textarea>

            </div>
            <div className="form-check d-flex justify-content-center mb-4">
                <input className="form-check-input me-2" type="checkbox" value="" id="form4Example4" checked />
                <label className="form-check-label" for="checkbox">
                    Receber promoções
                </label>
            </div>

            
            <button type="submit" className="btn btn-primary btn-block mb-4">Enviar</button>
        </form>
        </div>
    </section>
    )
}

export default FormContato